          ...
          {skills.map((skill, index) => (
            <Card3D key={skill.label} intensity={25} className="relative">
              <motion.div
                ...
                className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-6 hover:bg-white/10 hover:border-amber-400/30 transition-all duration-300 group h-full"
              >
                <motion.div 
                  className="w-12 h-12 rounded-lg bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center mb-4 shadow-lg shadow-amber-500/20 group-hover:shadow-amber-500/40 transition-shadow"
                  whileHover={{ rotate: 360, scale: 1.1 }}
                  transition={{ duration: 0.5 }}
                >
                  <skill.icon className="w-6 h-6 text-slate-900" />
                </motion.div>
                <h4 className="font-semibold text-white mb-3 group-hover:text-amber-200 transition-colors">
                  {skill.label}
                </h4>
                <p className="text-sm text-white/70 leading-relaxed break-words">{skill.stack}</p>
              </motion.div>
            </Card3D>
          ))}
        </div>
