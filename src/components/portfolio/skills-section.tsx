...
export function SkillsSection() {
  ...
  return (
    <section ref={containerRef} className="relative min-h-screen overflow-x-hidden bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950" style={{ paddingTop: "4rem", paddingBottom: "4rem" }}>
      ...
      {/* Content */}
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full box-border">
        <motion.div
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 w-full" style={{ perspective: "1000px" }}>
          {skills.map((skill, index) => (
            <div key={skill.label} className="w-full min-w-0">
              <Card3D intensity={15} className="relative w-full">
                <motion.div
                  ...
                  className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-xl p-4 sm:p-6 hover:bg-white/10 hover:border-amber-400/30 transition-all duration-300 group h-full w-full box-border overflow-hidden"
                >
                  <motion.div 
                  ...
                  <h4 className="font-semibold text-white mb-2 sm:mb-3 group-hover:text-amber-200 transition-colors text-base sm:text-lg break-words overflow-wrap-anywhere">
                    {skill.label}
                  </h4>
                  <p className="text-xs sm:text-sm text-white/70 leading-relaxed break-words overflow-wrap-anywhere">{skill.stack}</p>
                </motion.div>
              </Card3D>
            </div>
          ))}
